...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None):
        ...
        if labels is None:
            if self.num_images == 1 and self.iDLG:
                ...
                print(f"Reconstructed Labels: {labels}")
                ...
                with torch.no_grad():
                    ...
                    if (iteration + 1) % 100 == 0:
                        if self.config['filter'] == 'none':
                            ...
                            raise ValueError()
                    if iteration % 1000 == 0:
                        print(f"Difference between current and previous version: {torch.nn.functional.mse_loss(prev_x_trial, x_trial)}")
                        prev_x_trial = x_trial.clone().detach()
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, label):
        ...
        def closure():
            ...
            # print(f"Gradient Norm of x_trial: {torch.sum(torch.tensor([torch.linalg.norm(g) for g in gradient]))}, \
            #       Gradient Norm of input: {torch.sum(torch.tensor([torch.linalg.norm(g) for g in input_gradient]))}")
            rec_loss = reconstruction_costs([gradient], input_gradient,
                                            cost_fn=self.config['cost_fn'], indices=self.config['indices'],
                                            weights=self.config['weights'])

            if self.config['total_variation'] > 0:
                rec_loss += self.config['total_variation'] * TVP(x_trial)

            ##############################################################
            if self.config['bn_loss'] > 0:
                # Sweep over normalization layers (BatchNorm / LayerNorm),
                # capture their activations for the current x_trial via
                ...
                def _hook(module, inp, out):
                    # Preserve the activation tensor (do NOT detach) so the
                    # BN loss can backpropagate to `x_trial`. Handle cases
                    # where `out` may be a tuple/list by selecting the first
                    # tensor output.
                    try:
                ...
                # Register hooks on LayerNorm and BatchNorm modules
                ...
                try:
                    ...
                    _ = self.model(x_trial)
                finally:
                    # Remove all hooks
                    for h in self.hooks:
                        h.remove()

                # If we didn't catch any normalization activations, fall back
                # to the global activation x_trial and use default targets.
                if len(self.activations) == 0:
                    print("Warning: No BN/LN layers with running stats found; ")
                    self.activations = [x_trial]
                    self.modules_with_stats = [None]

                # Compute per-layer BNLoss using running stats when
                # available, otherwise fall back to (0, 1).
                bn_losses = []
