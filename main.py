...
# Configure logging
if not os.path.exists(config.DEFAULT_LOG_PATH):
    os.makedirs(config.DEFAULT_LOG_PATH)

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler(os.path.join(config.DEFAULT_LOG_PATH, f'april_{datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.log')),
